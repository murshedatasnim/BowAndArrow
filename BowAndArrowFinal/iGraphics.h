...
void iShowBMP2(int x, int y, char filename[], int ignoreColor)
{
    ...
    // Written by Prof. Ashikur Rahman, tested on CodeBlocks
    // Load the BMP file
    ...
    for (int i = 0; i < nPixels; i++){
        ...
        pi &= 0x00FFFFFF;
        //Sets the alpha buffer value. if matches with the color that has to be transparent
        // replace it with background color otherwise leave it as is
        if (ignoreColor == pi){
                int rgb[3];
